...
// since 가 있으면 그 시각(ms) 이후 방문만 준다("지금부터 보기" 용).
export async function readVisits({ limit = 50, offset = 0, since = 0, scan = 6000 } = {}) {
  let stdout;
  try {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {   // 최근 것부터
    ...
    if (since && at && at < since) continue;                // '지금부터 보기' 기준 이전은 건너뜀
    out.push({
      at,
      ip,
