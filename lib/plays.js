...
export function openPlaysStore(db) {
  ...
  const q = prepareAll(db, {
    ...
    // 관리 화면 목록에서만 빼는 모드. 봇전은 전용 패널이 따로 있어 여기까지
    // 뜨면 같은 판이 두 번 보인다. 판수·랭킹·칭호에는 그대로 들어간다.
    pageSkip: `SELECT p.id, p.at, p.name, p.user_id, p.seconds, p.mobile, p.mode,
           ...
           FROM plays p WHERE p.mode IS NOT 'bot' ORDER BY p.at DESC LIMIT ? OFFSET ?`,
    countSkip: `SELECT COUNT(*) n FROM plays WHERE mode IS NOT 'bot'`,
  ...
  return {
    ...
    // 관리 화면용 페이지. { rows, total }
    // skipBot 이면 봇전을 뺀다 — 봇전 패널이 따로 있어 목록이 겹친다.
    page(limit = 20, offset = 0, skipBot = false) {
      return {
        rows: (skipBot ? q.pageSkip : q.page).all(limit, offset).map(shape),
        total: (skipBot ? q.countSkip : q.count).get().n
      };
    },
