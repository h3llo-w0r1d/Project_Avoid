...
export function openPlaysStore(db) {
  ...
  // 접속 국가(ISO 코드, GeoLite2). 예전 줄은 NULL 이라 화면엔 '–' 로 보인다.
  try { db.exec('ALTER TABLE plays ADD COLUMN country TEXT'); } catch { /* 이미 있음 */ }
  // 통신사(ASN 조직명, GeoLite2)와 브라우저 언어(두 글자). 예전 줄은 둘 다 NULL.
  try { db.exec('ALTER TABLE plays ADD COLUMN isp TEXT'); } catch { /* 이미 있음 */ }
  try { db.exec('ALTER TABLE plays ADD COLUMN lang TEXT'); } catch { /* 이미 있음 */ }

  const q = prepareAll(db, {
    insert: 'INSERT INTO plays (id, at, name, user_id, seconds, mobile, mode, country, isp, lang) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    // 관리 화면 목록에서만 빼는 모드. 봇전·1대1 은 전용 패널(봇전 기록·대전
    // 기록)이 따로 있어 여기까지 뜨면 같은 판이 두 번 보인다.
    // 판수·랭킹·칭호에는 그대로 들어간다.
    pageSkip: `SELECT p.id, p.at, p.name, p.user_id, p.seconds, p.mobile, p.mode, p.country, p.isp, p.lang,
           ...
           FROM plays p WHERE p.mode IS NOT 'bot' AND p.mode IS NOT 'versus'
           ORDER BY p.at DESC LIMIT ? OFFSET ?`,
    countSkip: `SELECT COUNT(*) n FROM plays
                WHERE mode IS NOT 'bot' AND mode IS NOT 'versus'`,
    page: `SELECT p.id, p.at, p.name, p.user_id, p.seconds, p.mobile, p.mode, p.country, p.isp, p.lang,
             (p.at = (SELECT MIN(at) FROM plays WHERE name = p.name)) AS first,
             (SELECT COUNT(DISTINCT date((at + 32400000)/1000, 'unixepoch'))
                FROM plays p2 WHERE p2.name = p.name AND p2.at <= p.at) AS day_index
    ...
    // 초 합계. 판수와 같은 자리에서 같은 방식으로 센다(이름 / 계정 각각).
    secondsByName: 'SELECT name, SUM(seconds) s FROM plays GROUP BY name',
    secondsByUser: 'SELECT user_id, SUM(seconds) s FROM plays WHERE user_id IS NOT NULL GROUP BY user_id',
  ...
  const shape = (r) => ({
    ...
    country: r.country ?? null,  // null = country 칸을 만들기 전의 옛 기록
    isp: r.isp ?? null,          // null = isp 칸을 만들기 전의 옛 기록
    lang: r.lang ?? null,        // null = lang 칸을 만들기 전의 옛 기록
    first: r.first === 1,        // 이 이름의 첫 판(첫 방문)
    dayIndex: r.day_index ?? 1   // 이 판이 그 이름의 몇 번째 접속 날인지
  });

  return {
    add({ name, seconds, userId = null, mobile = null, mode = null, country = null, isp = null, lang = null }) {
      const m = mobile == null ? null : (mobile ? 1 : 0);
      q.insert.run(globalThis.crypto.randomUUID(), Date.now(), name, userId, seconds, m, mode, country, isp, lang);
    ...
    // 이름 → 총 플레이 초. 판수의 초 판(版)이다.
    secondsByName() {
      const m = new Map();
      for (const r of q.secondsByName.all()) m.set(r.name, r.s ?? 0);
      return m;
    },

    // 계정 id → 총 플레이 초. 닉네임을 바꿔도 안 새므로 계정 쪽은 이걸 쓴다.
    secondsByUser() {
      const m = new Map();
      for (const r of q.secondsByUser.all()) m.set(r.user_id, r.s ?? 0);
      return m;
    },
    ...
    // 관리 화면용 페이지. { rows, total }
    // skipOwnPanel 이면 전용 패널이 있는 모드(봇전·1대1)를 뺀다.
    page(limit = 20, offset = 0, skipOwnPanel = false) {
      return {
        rows: (skipOwnPanel ? q.pageSkip : q.page).all(limit, offset).map(shape),
        total: (skipOwnPanel ? q.countSkip : q.count).get().n
      };
    },
