...
export function openPlaysStore(db) {
  ...
  const q = prepareAll(db, {
    insert: 'INSERT INTO plays (id, at, name, user_id, seconds, mobile, mode, country) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    // 판수·랭킹·칭호에는 그대로 들어간다.
    pageSkip: `SELECT p.id, p.at, p.name, p.user_id, p.seconds, p.mobile, p.mode, p.country,
    ...
    page: `SELECT p.id, p.at, p.name, p.user_id, p.seconds, p.mobile, p.mode, p.country,
             (p.at = (SELECT MIN(at) FROM plays WHERE name = p.name)) AS first,
             (SELECT COUNT(DISTINCT date((at + 32400000)/1000, 'unixepoch'))
                FROM plays p2 WHERE p2.name = p.name AND p2.at <= p.at) AS day_index
  ...
  return {
    add({ name, seconds, userId = null, mobile = null, mode = null, country = null }) {
      const m = mobile == null ? null : (mobile ? 1 : 0);
      q.insert.run(globalThis.crypto.randomUUID(), Date.now(), name, userId, seconds, m, mode, country);
