...
export function openPlaysStore(db) {
  ...
  const q = prepareAll(db, {
    insert: 'INSERT INTO plays (id, at, name, user_id, seconds) VALUES (?, ?, ?, ?, ?)',
    // 최근 CAP 개만 남긴다. CAP 번째 뒤(오래된 것)를 지운다.
    trim: `DELETE FROM plays WHERE at < (
             SELECT at FROM plays ORDER BY at DESC LIMIT 1 OFFSET ?
           )`,
    // 시간 역순 한 쪽
    page: 'SELECT id, at, name, user_id, seconds FROM plays ORDER BY at DESC LIMIT ? OFFSET ?',
  ...
  return {
    add({ name, seconds, userId = null }) {
      q.insert.run(globalThis.crypto.randomUUID(), Date.now(), name, userId, seconds);
