...
export function openSpinsStore(db) {
  ...
  const q = prepareAll(db, {
    insert: 'INSERT INTO spins (id, at, name, user_id, cost, reward) VALUES (?, ?, ?, ?, ?, ?)',
    trim: `DELETE FROM spins WHERE at < (
             SELECT at FROM spins ORDER BY at DESC LIMIT 1 OFFSET ?
           )`,
    page: `SELECT id, at, name, user_id, cost, reward
  ...
  const shape = (r) => ({
    ...
    cost: r.cost, reward: r.reward
  });

  return {
    add({ name, userId = null, cost, reward }) {
      q.insert.run(globalThis.crypto.randomUUID(), Date.now(), name, userId, cost, reward);
      q.trim.run(CAP);
    },
