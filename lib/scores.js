...
export async function openScoreStore(dir, shared = null) {
  ...
  const q = prepareAll(db, {
    ...
    // 이름별 총 판수(모드 합). 관리 화면 계정 목록에 '몇 판 했는지' 붙일 때 쓴다.
    runsByName: 'SELECT name, COALESCE(SUM(runs),0) n FROM scores GROUP BY name',
  ...
  return {
    ...
    // 이름 → 총 판수(모드 합) 맵. 계정 목록에 판수를 붙이는 데 쓴다.
    runsByName() {
      const m = new Map();
      for (const r of q.runsByName.all()) m.set(r.name, r.n);
      return m;
    },
