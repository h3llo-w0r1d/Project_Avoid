// 도전모드(탑 오르기)·봇전 기록. "누가 언제 무엇을 했고 성공했는지"를 시간순으로.
//
// 판정은 클라이언트가 하므로 이 기록도 참고용이다(코인·룰렛과 같은 신뢰 모델).
// 이름은 로그인한 사람은 서버 닉네임으로 덮어써 사칭을 막는다(server.js).

import { addColumnIfMissing, prepareAll } from './db.js';
...
-- 만드라이더 완주 기록. 맵마다 길이가 달라 어느 코스를 달렸는지 같이 남긴다.
CREATE TABLE IF NOT EXISTS mandrider_log (
  id      TEXT PRIMARY KEY,
  at      INTEGER NOT NULL,
  name    TEXT    NOT NULL,
  user_id TEXT,
  map     TEXT    NOT NULL,       -- 맵 이름(사람이 읽는 쪽)
  seconds REAL    NOT NULL,       -- 그 판에서 달린 시간
  best    INTEGER NOT NULL,       -- 1이면 그 계정의 최고 기록을 갈아치운 판
  died    INTEGER NOT NULL DEFAULT 0  -- 1이면 붉은 만드라고라에 잡혀 끝난 판
);
CREATE INDEX IF NOT EXISTS mandrider_log_at ON mandrider_log(at DESC);
`;
...
export function openModeLogs(db) {
  db.exec(SCHEMA);
  // 먼저 만들어진 표에는 칸이 없다. CREATE TABLE IF NOT EXISTS 는 이미 있는
  // 표를 손대지 않으므로, 나중에 더한 칸은 이렇게 따로 채워 넣어야 한다.
  addColumnIfMissing(db, 'mandrider_log', 'died', 'INTEGER NOT NULL DEFAULT 0');
  return {
    ...
    mandrider: makeStore(db, 'mandrider_log', ['map', 'seconds', 'best', 'died'], (r) => ({
      id: r.id, at: r.at, name: r.name, member: r.user_id != null,
      map: r.map, seconds: r.seconds, best: r.best === 1, died: r.died === 1
    }))
  };
}
