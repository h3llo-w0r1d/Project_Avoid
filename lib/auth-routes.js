...
export function attachAuth(app, users, options = {}) {
  ...
  app.get('/auth/:name', (req, res) => {
    ...
    if (!cfg) return res.status(404).send(t(req, 'api.providerDisabled'));
  ...
  const callback = async (req, res) => {
    ...
    if (source.error) return fail(req, res, 'api.loginCancelled');
    if (!consumeState(req, res, source.state)) {
      return fail(req, res, 'api.loginExpired');
    }
    if (!source.code) return fail(req, res, 'api.noAuthCode');
  ...
  app.post('/api/me/nickname', async (req, res) => {
    ...
    if (!result.ok) return res.status(400).json({ error: t(req, result.reason, result.vars) });
    if (result.name === '익명') {
      return res.status(400).json({ error: t(req, 'api.nicknameRequired') });
    }
    if (users.nicknameTaken(result.name, req.user.id)) {
      return res.status(409).json({ error: t(req, 'api.nicknameTaken') });
    }
...
function fail(req, res, key) {
  res.redirect(`/?authError=${encodeURIComponent(t(req, key))}`);
}
