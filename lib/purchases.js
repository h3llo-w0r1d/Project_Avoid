...
// 상점에서 산 것인지 가리는 조건. 캐릭터는 접두사가 없다.
const SHOP_WHERE = "character LIKE 'trail:%' OR character LIKE 'arena:%' OR character LIKE 'fx:%'";
...
export function openPurchasesStore(db) {
  ...
  const q = prepareAll(db, {
    ...
    // 상점(발자국 효과·경기장 스킨)은 character 에 'trail:' 'arena:' 를 붙여
    // 넣는다. 캐릭터 구매와 한 표를 쓰되 관리 화면에서는 갈라 본다.
    // ('fx:' 는 항목을 나누기 전에 쓰던 옛 접두사라 함께 걸러 준다.)
    pageShop: `SELECT id, at, name, user_id, character, char_name, cost
               FROM purchases WHERE ${SHOP_WHERE} ORDER BY at DESC LIMIT ? OFFSET ?`,
    countShop: `SELECT COUNT(*) n FROM purchases WHERE ${SHOP_WHERE}`,
    pageChar: `SELECT id, at, name, user_id, character, char_name, cost
               FROM purchases WHERE NOT (${SHOP_WHERE}) ORDER BY at DESC LIMIT ? OFFSET ?`,
    countChar: `SELECT COUNT(*) n FROM purchases WHERE NOT (${SHOP_WHERE})`,
  ...
  return {
    ...
    // kind: 'all' | 'char'(캐릭터만) | 'shop'(상점만)
    page(limit = 20, offset = 0, kind = 'all') {
      const [pageQ, countQ] = kind === 'shop' ? [q.pageShop, q.countShop]
        : kind === 'char' ? [q.pageChar, q.countChar]
        : [q.page, q.count];
      return { rows: pageQ.all(limit, offset).map(shape), total: countQ.get().n };
    },
