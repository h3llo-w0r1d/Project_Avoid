...
export async function openDatabase(dir) {
  ...
  // 지인용 표시: 탑의 모든 층을 관리자처럼 열어 주되 탑 랭킹에는 안 잡히게 한다.
  // 기본값 0(꺼짐)이라 기존 계정은 영향이 없다.
  addColumnIfMissing(db, 'users', 'tower_free', 'INTEGER NOT NULL DEFAULT 0');
  // 만드라이더 최고 기록(밀리초). 0 은 아직 안 달렸다는 뜻이다 — 짧을수록
  // 좋은 기록이라, '없음' 을 0 으로 두면 랭킹에서 거르기만 하면 된다.
  addColumnIfMissing(db, 'users', 'mandrider_ms', 'INTEGER NOT NULL DEFAULT 0');

  // ── 지갑. 원래 브라우저(localStorage)에만 뒀는데, 그러면 같은 계정이라도
  // 기기를 바꾸면 코인과 산 것들이 안 따라온다("폰으로 들어오니 캐릭터가
  // 사라져요" 제보). 계정에 붙여 어느 기기에서든 같은 지갑을 쓰게 한다.
  addColumnIfMissing(db, 'users', 'coins', 'INTEGER NOT NULL DEFAULT 0');
  addColumnIfMissing(db, 'users', 'owned', 'TEXT');          // 코인으로 산 캐릭터 id (JSON)
  addColumnIfMissing(db, 'users', 'owned_fx', 'TEXT');       // 산 발자국 효과 (JSON)
  addColumnIfMissing(db, 'users', 'owned_arena', 'TEXT');    // 산 경기장 스킨 (JSON)
  addColumnIfMissing(db, 'users', 'playtime', 'REAL NOT NULL DEFAULT 0');   // 누적 게임 시간(초)
  addColumnIfMissing(db, 'users', 'spins_used', 'INTEGER NOT NULL DEFAULT 0');
  // 발자국 효과를 켠 채로 버틴 누적 시간(효과 id → 초). 단계를 올리는 값이다.
  addColumnIfMissing(db, 'users', 'fx_time', 'TEXT');
