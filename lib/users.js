...
export async function openUserStore(dir, shared = null) {
  ...
  const q = prepareAll(db, {
    ...
    // 지인용 표시: 탑 전체를 열어 주되 탑 랭킹에서는 뺀다.
    setTowerFree: 'UPDATE users SET tower_free = ? WHERE id = ?',
    // 룰렛 희귀 보상(0.1~3%)을 뽑은 누적 횟수 +1
    bumpLucky: 'UPDATE users SET lucky_hits = lucky_hits + 1 WHERE id = ?',
    touchSeen: 'UPDATE users SET last_seen = ? WHERE id = ?',

    // 지갑 — 코인과 산 것들. 계정에 붙여 기기가 바뀌어도 따라오게 한다.
    saveWallet: `UPDATE users SET coins = ?, owned = ?, owned_fx = ?, owned_arena = ?,
                   playtime = ?, spins_used = ?, fx_time = ? WHERE id = ?`,
    ...
    // tower_free 는 지인용으로 랭킹만 빼는 표시라 두 쿼리 모두에서 걸러야
    // 순위 계산(towerRankOf)에서도 안 끼어 다른 사람 순위가 밀리지 않는다.
    towerRanking: `SELECT id, nickname AS name, challenge AS floor
                   FROM users
                   WHERE nickname IS NOT NULL AND challenge > 0 AND tower_free = 0
                   ORDER BY challenge DESC, created_at ASC
                   LIMIT ?`,
    towerRankOf: `SELECT COUNT(*) n FROM users
                  WHERE nickname IS NOT NULL AND challenge > ? AND tower_free = 0`,

    // 만드라이더 랭킹 — 통산 최고 기록. 짧을수록 위고, 같으면 먼저 세운 쪽이 위다.
    // 기록이 없으면 0 이라 목록과 순위 계산 양쪽에서 걸러 낸다.
    setMandrider: `UPDATE users SET mandrider_ms =
                     CASE WHEN mandrider_ms = 0 OR ? < mandrider_ms THEN ? ELSE mandrider_ms END
                   WHERE id = ?`,
    mandriderRanking: `SELECT id, nickname AS name, mandrider_ms AS ms
                       FROM users
                       WHERE nickname IS NOT NULL AND mandrider_ms > 0
                       ORDER BY mandrider_ms ASC, created_at ASC
                       LIMIT ?`,
    mandriderRankOf: `SELECT COUNT(*) n FROM users
                      WHERE nickname IS NOT NULL AND mandrider_ms > 0 AND mandrider_ms < ?`,
  ...
  const shape = (r) => {
    ...
    return {
      ...
      mandriderMs: r.mandrider_ms ?? 0,     // 만드라이더 최고 기록(ms, 0 이면 없음)
      towerFree: !!r.tower_free,            // 지인용: 탑 전체 개방 + 랭킹 제외
      luckyHits: r.lucky_hits ?? 0,         // 룰렛 희귀 보상 누적 횟수
      lastSeen: r.last_seen ?? null,        // 마지막 접속 시각
      wallet: {                             // 계정 지갑(기기가 바뀌어도 따라온다)
        coins: Math.max(0, Math.floor(r.coins ?? 0)),
        owned: parseTitles(r.owned),          // 코인으로 산 캐릭터
        ownedFx: parseTitles(r.owned_fx),     // 산 발자국 효과
        ownedArena: parseTitles(r.owned_arena),   // 산 경기장 스킨
        playtime: Math.max(0, Number(r.playtime) || 0),
        spinsUsed: Math.max(0, Math.floor(r.spins_used ?? 0)),
        // 발자국 효과를 켠 채로 버틴 누적 시간(효과 id → 초)
        fxTime: parseMap(r.fx_time)
      }
    };
  };

  // { 키: 숫자 } 꼴 JSON 을 읽는다. 비었거나 깨졌으면 빈 객체.
  function parseMap(raw) {
    ...
    try {
      ...
      for (const [k, n] of Object.entries(v)) {
        const num = Number(n);
        if (Number.isFinite(num) && num > 0) out[String(k)] = num;
      }
      return out;
    } catch { return {}; }
  }
  ...
  function cleanMap(m) {
    ...
    for (const [k, n] of Object.entries(m)) {
      const num = Number(n);
      if (Number.isFinite(num) && num > 0) out[String(k)] = Math.round(num * 100) / 100;
    }
    return out;
  }
  ...
  function addMaps(a, b) {
    const out = { ...cleanMap(a) };
    for (const [k, n] of Object.entries(cleanMap(b))) out[k] = (out[k] ?? 0) + n;
    return out;
  }
  ...
  return {
    ...
    // 만드라이더 랭킹 상위 목록.
    mandriderRanking(limit = 100) {
      return q.mandriderRanking.all(limit);
    },

    // 그 기록이면 몇 위인지(나보다 빠른 사람 수 + 1).
    mandriderRankOf(ms) {
      if (!(ms > 0)) return null;
      return q.mandriderRankOf.get(ms).n + 1;
    },

    // 만드라이더 최고 기록을 줄인다(늘어나지 않는다). 새 최고 기록을 돌려준다.
    setMandriderBest(userId, ms) {
      const best = Math.max(1, Math.round(ms));
      q.setMandrider.run(best, best, userId);
      return shape(q.byId.get(userId))?.mandriderMs ?? 0;
    },
    ...
    // 지인용 표시를 켜고 끈다. 켜면 관리자처럼 탑 전체가 열리고 탑 랭킹에서 빠진다.
    setTowerFree(userId, on) {
      q.setTowerFree.run(bit(on), userId);
      return shape(q.byId.get(userId));
    },
    ...
    // 계정 지갑을 통째로 저장한다. 쓰거나 사면 줄어들 수도 있어야 하므로
    // 그대로 덮어쓴다(합치기는 아래 mergeWallet 이 따로 한다).
    saveWallet(id, w) {
      const list = (v) => JSON.stringify(Array.isArray(v) ? [...new Set(v.map(String))] : []);
      q.saveWallet.run(
        Math.max(0, Math.floor(Number(w?.coins) || 0)),
        list(w?.owned), list(w?.ownedFx), list(w?.ownedArena),
        Math.max(0, Number(w?.playtime) || 0),
        Math.max(0, Math.floor(Number(w?.spinsUsed) || 0)),
        JSON.stringify(cleanMap(w?.fxTime)),
        id
      );
      return shape(q.byId.get(id))?.wallet ?? null;
    },

    // 기기에 있던 지갑을 계정에 합친다.
    //
    //
    // 빼거나 덮어쓰지 않는다 — 더하기와 합집합만 한다. 그래서 어느 쪽도
    // 잃지 않는다(PC 500 + 폰 100 = 600). 룰렛도 누적 시간과 쓴 횟수를
    // 함께 더해야 '남은 횟수' 가 그대로 보존된다.
    //
    // 두 번 합쳐지면 코인이 부풀므로, 기기마다 한 번만 부르는 건 클라가
    // 표시를 남겨 관리한다(avoidarc.merged.<계정id>).
    mergeWallet(id, local) {
      const cur = shape(q.byId.get(id))?.wallet;
      if (!cur) return null;
      const uni = (a, b) => [...new Set([...(a ?? []), ...(Array.isArray(b) ? b.map(String) : [])])];
      return this.saveWallet(id, {
        coins: cur.coins + Math.max(0, Math.floor(Number(local?.coins) || 0)),
        owned: uni(cur.owned, local?.owned),
        ownedFx: uni(cur.ownedFx, local?.ownedFx),
        ownedArena: uni(cur.ownedArena, local?.ownedArena),
        playtime: cur.playtime + Math.max(0, Number(local?.playtime) || 0),
        spinsUsed: cur.spinsUsed + Math.max(0, Math.floor(Number(local?.spinsUsed) || 0)),
        // 효과별 누적 시간도 키마다 더한다 — 기기에서 쌓은 만큼이 살아난다
        fxTime: addMaps(cur.fxTime, local?.fxTime)
      });
    },

    // 토큰으로 사용자를 찾는다. 만료됐으면 null.
    // 겸사겸사 '최근 접속' 시각을 남긴다. 요청마다 쓰면 낭비라 10분에 한 번만.
    userForToken(token) {
      if (!token) return null;
      const u = shape(q.sessionUser.get(token, Date.now())) ?? null;
      if (u) {
        const now = Date.now();
        if (!u.lastSeen || now - u.lastSeen > 10 * 60 * 1000) {
          q.touchSeen.run(now, u.id);
          u.lastSeen = now;
        }
      }
      return u;
