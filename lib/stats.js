...
// 평균 기록에 넣을 최소 생존 시간(초). 이 미만은 '시작하자마자 죽은 판'으로 보고
// 평균에서 뺀다(첫 빔에 바로 죽는 3~4초대). 값만 바꾸면 기준이 바뀐다.
const REAL_MIN = 4;
...
export function openStatsStore(db) {
  ...
  // 그날의 최고 생존 기록(초). REAL 이라 addCol(INTEGER)과 따로 넣고, 처음
  // 추가할 때 기존 날짜는 plays 로그에서 한 번 백필한다(KST 날짜로 묶어 MAX).
  if (!cols.includes('best')) {
    db.exec('ALTER TABLE daily ADD COLUMN best REAL NOT NULL DEFAULT 0');
    try {
      db.exec(`UPDATE daily SET best = COALESCE(
        (SELECT MAX(seconds) FROM plays
           WHERE date((at + 32400000)/1000, 'unixepoch') = daily.day), 0)`);
    } catch { /* plays 표가 없으면 백필 생략 */ }
  }

  // '제대로 한 판'(REAL_MIN 초 이상)의 합·개수. 시작하자마자 죽은 짧은 판을
  // 평균 기록에서 빼기 위해 따로 센다. 과거 날짜는 plays 에서 백필한다.
  if (!cols.includes('real_seconds')) {
    db.exec('ALTER TABLE daily ADD COLUMN real_seconds REAL NOT NULL DEFAULT 0');
    db.exec('ALTER TABLE daily ADD COLUMN real_runs INTEGER NOT NULL DEFAULT 0');
    try {
      db.exec(`UPDATE daily SET
        real_seconds = COALESCE((SELECT SUM(seconds) FROM plays
          WHERE date((at + 32400000)/1000, 'unixepoch') = daily.day AND seconds >= ${REAL_MIN}), 0),
        real_runs = COALESCE((SELECT COUNT(*) FROM plays
          WHERE date((at + 32400000)/1000, 'unixepoch') = daily.day AND seconds >= ${REAL_MIN}), 0)`);
    } catch { /* plays 표가 없으면 백필 생략 */ }
  }
  ...
  const q = prepareAll(db, {
    ...
    addBest: 'UPDATE daily SET best = MAX(best, ?) WHERE day = ?',
    // 4초 이상만 '제대로 한 판'으로 따로 센다(평균 기록용).
    addReal: 'UPDATE daily SET real_seconds = real_seconds + ?, real_runs = real_runs + 1 WHERE day = ?',
    ...
    byMonth: `SELECT substr(day, 1, 7) AS month,
                     ...
                     MAX(best) best,
                     SUM(real_seconds) real_seconds, SUM(real_runs) real_runs
  ...
  return {
    ...
    runFinished(seconds, at = Date.now()) {
      ...
      q.addBest.run(seconds, day);   // 그날 최고 기록 갱신
      if (seconds >= REAL_MIN) q.addReal.run(seconds, day);   // 짧은 판 제외한 평균용
    },
    ...
    recent(days = 14) {
      return q.recent.all(days).map((row) => {
        ...
        return {
          ...
          best: Math.round((row.best ?? 0) * 100) / 100,   // 그날 최고 생존(초)
          realSeconds: Math.round(row.real_seconds ?? 0),  // 4초 이상 판 합(평균용)
          realRuns: row.real_runs ?? 0,                    // 4초 이상 판 수
    ...
    monthly(months = 24) {
      return q.byMonth.all(months).map((row) => {
        ...
        return {
          ...
          runs: row.runs,
          finished: row.finished,
          best: Math.round((row.best ?? 0) * 100) / 100,   // 그 달 최고 생존(초)
          realSeconds: Math.round(row.real_seconds ?? 0),
          realRuns: row.real_runs ?? 0,
          seconds: Math.round(row.seconds),
          players: q.players.get(from, to).n,
          members: q.members.get(from, to).n
        };
