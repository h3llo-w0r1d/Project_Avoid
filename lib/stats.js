...
export function openStatsStore(db) {
  ...
  // 예전 DB 에는 uniques 칸이 없다. 없으면 더해 준다.
  const hasUniques = db.prepare('PRAGMA table_info(daily)').all()
    .some((c) => c.name === 'uniques');
  if (!hasUniques) {
    db.exec('ALTER TABLE daily ADD COLUMN uniques INTEGER NOT NULL DEFAULT 0');
  }
  ...
  const q = prepareAll(db, {
    ...
    byMonth: `SELECT substr(day, 1, 7) AS month,
                     SUM(visits) visits, SUM(uniques) uniques, SUM(runs) runs,
                     SUM(finished) finished, SUM(seconds) seconds
