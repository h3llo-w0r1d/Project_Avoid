...
export const TITLES = [
  ...
  // 기록 칭호 — 버티기 최고 기록으로 얻는다. 판수와 달리 오래 한다고
  // 얻어지지 않아서, 달고 있으면 실력이 바로 읽힌다.
  // announce: 새로 내놓은 칭호. 조건을 이미 채워 둔 사람에게 접속할 때
  // 한 번 알려 준다 — 안 그러면 생긴 줄도 모르고 지나간다.
  { id: 'wall100', name: '100초의 벽', icon: '', bestTime: 100, announce: true,
    cond: '버티기 100초 돌파', color: '#4fd6ff' },
...
export function isEarned(title, ctx) {
  ...
  // 버티기 최고 기록으로 얻는 칭호(100초의 벽)
  if (title.bestTime) return (ctx.bestTime ?? 0) >= title.bestTime;
