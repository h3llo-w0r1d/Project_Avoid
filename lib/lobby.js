...
export function attachLobby(httpServer, users = null, matchLog = null, plays = null,
  ...
  function recordResult(match, a, b, mode) {
    // 관리자(운영자) 판은 어디에도 안 남긴다 — 판수·전적·대전 기록 전부.
    ...
    for (const c of [a, b]) {
      ...
      // 초는 소수 둘째 자리까지. 다른 기록과 같은 정밀도로 맞춘다.
      try { plays?.add({ name: c.name,
        seconds: Math.round((match.elapsed ?? 0) * 100) / 100,
        userId: c.userId ?? null, mode: 'versus' }); }
      catch (err) { console.error('대전 판수 기록 실패:', err); }
    }
    ...
    // 한쪽이라도 관리자면 전적·대전 기록을 남기지 않는다.
    if (admin(winner) || admin(loser)) return;
