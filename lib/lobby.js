...
export function attachLobby(httpServer, users = null, matchLog = null, plays = null,
  isAdmin = () => false) {
  ...
  function recordResult(match, a, b, mode) {
    // 관리자(운영자)는 판수·전적에 안 들어간다. 다만 대전 기록에는 남긴다 —
    // 그건 "누가 누구와 몇 초 했나"를 살펴보는 감사 로그라, 안 남기면 관리자가
    // 자기 판을 어디서도 못 본다.
    const admin = (c) => !!c?.userId && isAdmin({ id: c.userId, nickname: c.name });
    const seconds = Math.round((match.elapsed ?? 0) * 100) / 100;

    // 판수는 관리자가 아닌 쪽만. 아래 return 들보다 앞에 둬야 무승부나
    // 게스트끼리 한 판도 세어진다 — 전적은 못 남겨도 한 판 한 건 맞다.
    for (const c of [a, b]) {
      if (!c || admin(c)) continue;
      try {
        plays?.add({ name: c.name, seconds, userId: c.userId ?? null, mode: 'versus' });
      } catch (err) {
        console.error('대전 판수 기록 실패:', err);
      }
    }
    ...
    // 전적(승패)은 관리자가 끼면 안 센다 — 랭킹이 흐려진다.
    if (!admin(winner) && !admin(loser)) {
  ...
  const handlers = {
    ...
    'room-join'(client, msg) {
      ...
      if (!room) return send(client, { type: 'room-error', error: t(client.langReq, 'api.roomNotFound') });
      if (room.host === client) return send(client, { type: 'room-error', error: t(client.langReq, 'api.roomIsMine') });
      if (room.guest) return send(client, { type: 'room-error', error: t(client.langReq, 'api.roomFull') });
  ...
  wss.on('connection', (socket, req) => {
    ...
    const account = users ? users.userForToken(cookieOf(req, 'session')) : null;
    // room-error 문구를 화면 언어로 내려면 t(req, 키) 가 필요한데, express
    // 미들웨어를 안 거치는 업그레이드 요청이라 req.cookies 가 없다. 접속
    // 시점에 한 번 흉내만 낸 req 를 만들어 붙여 둔다.
    const langReq = { cookies: { lang: cookieOf(req, 'lang') }, headers: req.headers };
  ...
  // 쿠키 헤더에서 값 하나만 꺼낸다. WebSocket 에는 cookie-parser 가
  // 끼어들 자리가 없어서 직접 판다(server.js 의 쿠키 미들웨어와 같은 방식).
