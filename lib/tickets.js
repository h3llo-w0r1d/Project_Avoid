...
export function createTickets({
  ...
  return {
    ...
    // 통과하면 { ok: true }, 아니면 { error: 'lib/strings.js 의 키' } —
    // 실제 문구는 부르는 쪽이 lib/i18n.js 의 t(req, error) 로 만든다.
    redeem(id, seconds, { ip, userId = null } = {}) {
      const t = live.get(id);
      if (!t) {
        return { error: 'api.ticketInvalid' };
      }
      ...
      if (now - t.at > maxAgeMs) {
        return { error: 'api.ticketExpired' };
      }
      ...
      if (seconds * 1000 > (now - t.at) + graceMs) {
        return { error: 'api.ticketTimeMismatch' };
      }
      return { ok: true };
    },
