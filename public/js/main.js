...
function applyArena(id) {
  ...
  // 하늘은 캔버스 뒤 CSS 배경 그림이다. 스킨마다 그림을 CSS 가 고른다(style.css #stage).
  document.documentElement.dataset.arena = spec.id;
}
...
(() => {
  ...
  const SEG = [
    ...
    { label: '나만의 캐릭터 제작', custom: true, color: '#ff6fae' }   // 커스텀 캐릭터 제작권
  ];
  // 보상별 확률(합 100). 한정 보상 셋이 각각 1% 아래에 몰려 있다:
  // 노래 0.1% · 나만의 캐릭터 제작 0.1% · 가나디라고라 1% · 은하수 1% · 코인 잭팟 2%.
  // 코인은 값이 클수록 희귀: 꽝 43.9 · 5코인 20 · 10코인 15 · 20코인 10 · 50코인 7.
  // (가나디 5 → 1 로 내리고 은하수 1 을 새로 뺐다. 남은 3 은 꽝으로: 40.9 → 43.9)
  // (나만의 캐릭터 제작 0.1 을 새로 넣으며 꽝에서 뗐다: 43.9 → 43.8)
  const WEIGHTS = [
    { coins: 0, p: 43.8 }, { coins: 5, p: 20 }, { coins: 10, p: 15 },
  ...
  function pick(table = WEIGHTS) {
    ...
    // 특별 칸을 여기서 빼지 않으면 코인 당첨이 그 칸에 멈춰 버린다.
    const idxs = SEG.map((s, i) => (!s.song && !s.lucky && !s.arena && !s.jackpot && !s.custom
      && s.coins === chosen.coins ? i : -1)).filter((i) => i >= 0);
...
const PHYS_TUNE = {
  ...
  // 빠르다 — 좁은 무대(반지름 11)에서 과속해 가장자리로 미끄러져 나간다. 처음 값(16.5)의 두 배.
  fast: { speed: 33, accel: 380 }
};
// 셋 다 — 가속은 slip 의 값을 쓴다. 과속의 가속까지 받으면 얼음 위라는 느낌이 사라진다.
// 속도는 과속 층의 절반(16.5)으로 묶는다. 얼음 위에서 33 이면 손을 떼는 순간
// 무대 지름(22)의 두 배를 미끄러져 그냥 떨어진다.
PHYS_TUNE.all = { ...PHYS_TUNE.slip, ...PHYS_TUNE.heavy, speed: 16.5 };

// 코인 층에서만 쓰는 스폰 속도. 평소(7~12초에 하나)로는 층이 성립하지 않는다.
//
// 30초에 30개를 요구하는 층(27·30층)이 기준이다. 예전 값(1.2~2.2초에 하나)
// 으로는 30초 동안 17개밖에 안 떠서 아예 깰 수가 없었다. 스폰 규칙을 그대로
// 3,000판 돌려 보면 지금 값은 30초에 35개(안 먹고 둘 때) ~ 46개(부지런히
// 먹을 때) 가 뜬다 — 30개를 채우려면 뜬 것의 3분의 2를 주워야 한다.
const CHAL_COIN_RATE = { min: 0.45, max: 0.8, field: 6, life: 5 };
