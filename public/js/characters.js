...
export const CHARACTERS = [
  ...
  },
  {
    ...
    // lumpy 는 안 쓴다. roughen() 의 변형량이 절대값이라, 반지름 0.06 짜리
    // 가는 다리에선 굵기의 절반이 흔들려 다리가 끊긴 것처럼 보인다.
    // 게다가 감쇠식이 높이 1.3 기준이라 굵은 몸통엔 안 먹고 다리에만 몰린다.
    // 원본 그림도 매끈한 스티커 그림체라 안 넣는 편이 맞다.
    //
    // 산삼과 당근을 가르는 건 '어깨'다. 당근은 위에서 아래로 고르게 좁아지는
    // 원뿔이고, 산삼은 통통한 몸통이 어깨에서 뚝 끊겨 가늘고 긴 다리로 간다.
    // 그래서 y 0.68~0.90 을 급하게 좁히고, 그 아래는 거의 굵기 변화 없이
    // 길게 내렸다. 위(뇌두)는 둥글게 여민다.
    profile: [
      [0.000, 0.000], [0.090, 0.055], [0.220, 0.082], [0.380, 0.100],
      [0.540, 0.118], [0.680, 0.152], [0.790, 0.235], [0.900, 0.345],
      [1.010, 0.445], [1.130, 0.522], [1.270, 0.560], [1.420, 0.556],
      [1.560, 0.512], [1.690, 0.432], [1.800, 0.320], [1.890, 0.185],
      [1.950, 0.000]
    ],
    face: {
      ...
      eyeBulge: 1.3,
      eyeGap: 0.24,
      // 얼굴은 통통한 몸통 한가운데에 와야 한다. 기본값(눈 0.56)이면 몸통이
      // 좁아지는 어깨쯤에 걸려 얼굴이 다리로 흘러내린 것처럼 보인다.
      eyeY: 0.68,
      mouthY: 0.575,                     // 원본 그림처럼 눈 바로 아래 붙여서
