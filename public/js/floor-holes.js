// 하드코어 전용 — 무대 바깥 링의 바닥이 부분적으로 사라졌다 나타난다.
//
// 공정하게: 가운데(반경 R0 이내)는 절대 사라지지 않아 늘 도망갈 곳이 있다.
// 바깥 링을 부채꼴 타일로 나눠, 한두 개씩 "경고(빨갛게 금이 감) → 사라짐(구멍,
// 밟고 있으면 낙사) → 다시 생김" 순서로 돈다. 전기선의 예열처럼, 사라지기 전에
...
export class FloorHoles {
  ...
  reset() {
    ...
    this.spawnTimer = 1.5;   // 시작하고 잠깐 뒤 첫 구멍
  }
  ...
  #open() {
    ...
    const t = idle[(Math.random() * idle.length) | 0];
    t.phase = 'warn';
    t.t = 0;
  }
  ...
  update(dt) {
    ...
    for (const t of this.tiles) {
      ...
      if (t.phase === 'warn') {
        // 빨갛게 깜빡 — 사라지기 전 경고. 아직 밟아도 안전.
        t.mat.color.setHex(0xff5a3a);
        t.mat.opacity = 0.22 + 0.32 * Math.abs(Math.sin(now * 12));
        if (t.t >= WARN_T) { t.phase = 'open'; t.t = 0; }
      } else if (t.phase === 'open') {
        // 바닥이 사라졌다 — 어두운 구멍. 밟고 있으면 낙사(아래 isOpenAt).
        t.mat.color.setHex(0x0a0c12);
        const u = t.t / OPEN_T;
        t.mat.opacity = Math.min(0.9, u * 8, (1 - u) * 8 + 0.05);
        if (t.t >= OPEN_T) { t.phase = 'idle'; t.t = 0; t.mat.opacity = 0; }
      }
    }
  }

  // 지금 이 지점의 바닥이 사라져 있는가(낙사 판정용). 안쪽 안전지대는 늘 false.
  isOpenAt(x, z) {
