...
export const PLAYER = {
  ...
  // 세기를 아무리 키워도 실제로는 반 칸도 못 밀려났다 — 전속력으로 박아도
  // 맞은 쪽이 방향키를 잡고 있으면 0.29칸이었다. 그래서 밋밋했다.
  pushBase: 13.0,     // 닿기만 해도 튕겨 나가는 기본 세기 (약 1.4칸)
  pushScale: 0.7,     // 달려와 박은 속도에 비례해 더 세게 (전속력 박기 약 3.7칸)
  pushMax: 24.0,      // 총 세기 상한. 마주 보고 전속력으로 박는 경우를 막는다
  pushDecay: 60.0,    // 밀린 속도가 잦아드는 속도. 작을수록 멀리·오래 미끄러진다.
                      // 처음 튕기는 세기는 그대로 두고 이 값만 키우면,
                      // 맞는 순간의 반응은 살아 있으면서 거리만 줄어든다.
  pushGrip: 0.35      // 세게 밀리는 순간 남는 조작력(최고 속도 배율). 밀림이
                      // 잦아들면서 1 로 돌아온다. 1 이면 방향키로 다 버틴다.
};
