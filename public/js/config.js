...
export const PLAYER = {
  ...
  //
  // 밀린 속도는 보통 속도(vx/vz)와 섞지 않고 따로 둔다. 섞어 두었더니
  // 최고 속도 제한과 마찰(110)이 다음 프레임에 곧바로 깎아 버려서,
  // 세기를 아무리 키워도 실제로는 반 칸도 못 밀려났다. 그래서 밋밋했다.
  //
  // 어떻게 부딪히든 똑같이 1.5칸 밀려나게 맞춰 두었다. 그러려면 세 가지가
  // 다 필요하다 — 박은 속도를 안 더하고(pushScale 0), 부딪힌 순간 하던
  // 걸음을 멈추고(resolvePush), 밀리는 동안 조작을 막는다(pushGrip 0).
  // 셋 중 하나라도 빠지면 상황마다 거리가 제각각이 된다.
  pushBase: 13.0,     // 밀려나는 세기. 거리는 pushBase² / (2 × pushDecay) ≈ 1.4칸
  pushScale: 0,       // 달려와 박은 속도를 얼마나 더할지. 0 이면 언제나 같은 세기
  pushMax: 24.0,      // 밀림이 겹쳐 쌓일 때의 상한(연달아 부딪히는 경우)
  pushDecay: 60.0,    // 잦아드는 속도. 클수록 덜 밀린다. 밀리는 시간은 약 0.22초
  pushGrip: 0         // 밀리는 동안 남는 조작력(최고 속도 배율). 0 이면 버텨도
                      // 똑같이 밀려난다. 점프는 이것과 상관없이 그대로 된다.
};
...
export const COLORS = {
  ...
  haze: 0x3d3558    // 안개 = 지평선 색. 멀리 있는 나무가 여기로 녹아든다. 배경 그림의 보랏빛 구름에 맞췄다.
};
